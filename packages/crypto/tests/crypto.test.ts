...
describe('unlockVaultKey (AC1 — verification)', () => {
  ...
  it('correct password returns the vault key', async () => {
    const vaultKey = await unlockVaultKey(SYNTHETIC_PASSWORD, record, SYNTHETIC_USER_ID);
  ...
  it('wrong password throws authentication failure', async () => {
    await expect(
      unlockVaultKey(SYNTHETIC_PASSWORD_ALT, record, SYNTHETIC_USER_ID),
  ...
  it('empty password throws (different key → tag mismatch)', async () => {
    await expect(
      unlockVaultKey(Buffer.alloc(0), record, SYNTHETIC_USER_ID),
    ).rejects.toThrow(/authentication failed/i);
  });
  ...
  it('tampered salt breaks unlock (different derived key)', async () => {
    ...
    await expect(
      unlockVaultKey(SYNTHETIC_PASSWORD, tampered, SYNTHETIC_USER_ID),
    ).rejects.toThrow(/authentication failed/i);
  });
