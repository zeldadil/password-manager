...
CI: PR #30 — **10/10 checks pass on both pushes of this branch**:

| CI run | head | result |
|---|---|---|
| `35398221990` | `8ccdf87` (code + first evidence) | 10/10 pass |
| `35398455295` | `d8da408` (evidence: CI record) | 10/10 pass |

The `unit` job — where the sweep runs in CI, via `pnpm test:unit` — reported
`src/a11y/routes.a11y.test.tsx (19 tests)` and `Test Files 20 passed (20) · Tests 162 passed (162)`
→ `ci.txt`. Commits after `d8da408` are evidence-text only: they touch no code or test file.
