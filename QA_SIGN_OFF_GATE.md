...
| `R5_EVIDENCE_FILE_MISSING` | a named evidence file/directory **of the operative (newest) verdict** does not exist — in the worker's checkout or on any other ref of the repo | commit the artifact, fix the path, or name the CI run instead |
...
| `node scripts/qa/signoff-gate.selftest.mjs` | 49-case non-vacuity proof on a throwaway fixture board (every rule fires; every compliant control passes; the `t_5455942d` regressions are covered) |
...
**`R5_EVIDENCE_FILE_MISSING: … named in the verdict does not exist`** — check *which* verdict names the path. Only
the newest (operative) verdict is resolved; a path from a superseded verdict or another worker's handoff shows up as
`A5_EVIDENCE_SUPERSEDED` instead. If the operative verdict genuinely names a missing file, commit it, fix the path,
or cite the CI run URL. A path that exists only on another branch is accepted (`A4_EVIDENCE_OFF_TREE`).
...
| 5 | A comment that *quotes* the recommended `QA-VERDICT: …` line (e.g. an Architect handoff showing the exact command to paste) is parsed as a verdict. With `R5` scoped to the operative verdict the harm is now contained, but if such a quote is the *newest* record it becomes operative. Decide whether to ignore marker lines inside code spans/fences | `qa` |
