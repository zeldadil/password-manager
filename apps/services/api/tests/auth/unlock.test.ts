...
describe('BE-002b: POST /auth/unlock', () => {
  ...
  it('returns 401 for wrong master password', async () => {
    ...
    // BE-002e: generic error — must not distinguish wrong password from unknown user.
    expect(body.message).toBe('Invalid credentials');
  });

  // ── Negative: user not found ────────────────────────────────────────────────
  //
  // BE-002e: user enumeration prevention — unknown users get the SAME 401
  // "Invalid credentials" response as wrong passwords. Returning 404 here
  // would let an attacker enumerate valid emails/usernames.

  it('returns 401 (generic error) for unknown email — no user enumeration', async () => {
    const res = await server.inject({
      method: 'POST',
      url: '/auth/unlock',
      payload: {
  ...
  it('returns 401 (generic error) for unknown username — no user enumeration', async () => {
    const res = await server.inject({
      method: 'POST',
      url: '/auth/unlock',
      payload: {
  ...
  //
  // BE-002e: returns 401 (generic error) — not 404 — to prevent user enumeration.

  it('returns 401 (generic error) when both email and username are incorrect', async () => {
    const res = await server.inject({
      method: 'POST',
      url: '/auth/unlock',
      payload: {
  ...
  it('issue a different refresh token on each unlock attempt', async () => {
    ...
    // Access tokens may differ due to iat (issued-at) if unlocks span a second boundary.
