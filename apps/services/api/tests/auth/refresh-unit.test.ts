...
function isTokenExpired(row: { expiresAt: Date } | null | undefined): boolean {
  if (!row) return true;
  return row.expiresAt < new Date();
}

function isTokenRevoked(row: { revokedAt: Date | null } | null | undefined): boolean {
  if (!row) return true;
...
describe('BE-002f: token refresh — access token issuance', () => {
  ...
  it('access token signature uses constant-time comparison', () => {
    ...
    const token = signAccessToken('user-1', TEST_SECRET, ACCESS_TOKEN_TTL_SEC, 'sess-1');
