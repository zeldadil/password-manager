...
function makeDb(): { db: ReturnType<typeof drizzle<typeof schema>>; sql: Database.Database } {
...
function sessionIsValid(
  session: Pick<typeof sessions.$inferSelect, 'expiresAt' | 'deletedAt'>,
): boolean {
  const now = new Date();
...
async function findActiveSession(db: ReturnType<typeof drizzle<typeof schema>>, userId: string) {
...
describe('BE-002f: lock revocation', () => {
  ...
  describe('lock revocation flow', () => {
    it('lock revokes refresh token and soft-deletes session', async () => {
      ...
      const refreshTokenHash = 'aabbccdd'.repeat(8);
    ...
    it('lock is idempotent — revoking an already-revoked token is safe', async () => {
      ...
      const refreshTokenHash = 'eeffaabb'.repeat(8);
