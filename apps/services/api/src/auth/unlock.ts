...
import { unlockVaultKey, deriveVaultKey, type KdfRegistrationRecord } from '@password-manager/crypto';
import { signAccessToken, generateRefreshToken, hashRefreshToken, getJwtSecret } from './jwt';
...
export function unlockPlugin(server: FastifyInstance): void {
  server.post<{ Body: UnlockBody; Reply: UnlockResponse }>(
    '/auth/unlock',
    {
      schema: {
        body: {
    ...
    async (request: FastifyRequest<{ Body: UnlockBody }>, reply: FastifyReply) => {
      ...
      const accessToken = signAccessToken(userId, getJwtSecret(), ACCESS_TOKEN_TTL_SEC, sessionId);
