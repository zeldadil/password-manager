...
function buildClient(fetchImpl: typeof fetch, store = new InMemoryTokenStore(), extra: Partial<ConstructorParameters<typeof ApiClient>[0]> = {}) {
...
describe('ApiClient — refresh token handling', () => {
  it('refreshes the access token and retries the original request once on 401', async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse(envelope({ jwt: 'new-jwt', refreshToken: 'refresh-2' }, successHeader('RefreshSession'))),
      )
      .mockResolvedValueOnce(jsonResponse(envelope({ id: 'r1', name: 'Secret' }, successHeader('GetResource'))));
  ...
  it('does not attempt refresh on a 401 for a public path', async () => {
    ...
    await expect(client.post('/auth/login', { email: 'a@b.c', vaultKeyProof: 'p' })).rejects.toMatchObject({
      httpStatus: 401,
    });
  ...
  it('deduplicates concurrent refreshes into a single /auth/refresh call', async () => {
    ...
    const fetchMock = vi
      ...
      .mockImplementationOnce(async () => {
        ...
        return jsonResponse(envelope({ jwt: 'new-jwt', refreshToken: 'refresh-2' }, successHeader('RefreshSession')));
      })
  ...
  it('clears tokens and calls onUnauthorized when refresh fails', async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
        jsonResponse({ header: errorHeader('GetResource', 401), body: { errors: [] } }, { status: 401 }),
      )
      .mockResolvedValueOnce(
        jsonResponse({ header: errorHeader('RefreshSession', 401), body: { errors: [] } }, { status: 401 }),
      );
...
describe('ApiClient — error normalization', () => {
  it('normalizes a network failure into a network ApiError', async () => {
    ...
    await expect(client.get('/resources')).rejects.toMatchObject({ kind: 'network', message: 'Failed to fetch' });
  });
