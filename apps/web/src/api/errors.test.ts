...
describe('ApiError', () => {
  ...
  it('isUnauthorized is true for 401', () => {
    expect(new ApiError({ message: 'nope', kind: 'http', httpStatus: 401 }).isUnauthorized).toBe(true);
    expect(new ApiError({ message: 'nope', kind: 'unauthorized' }).isUnauthorized).toBe(true);
    expect(new ApiError({ message: 'nope', kind: 'http', httpStatus: 403 }).isUnauthorized).toBe(false);
  });
...
describe('apiErrorFromEnvelope', () => {
  ...
  it('falls back to a generic status message when nothing else is available', () => {
    expect(apiErrorFromEnvelope(500, undefined, undefined).message).toBe('Request failed with status 500');
    expect(apiErrorFromEnvelope(500, successHeader, undefined).message).toBe('Request failed with status 500');
  });
});
